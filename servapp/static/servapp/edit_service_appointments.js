...
document.addEventListener("DOMContentLoaded", function () {
    serviceCount = 0;
    activeIndex = 0;
    originalListingTitle = document.getElementById('title');
    loadServices();
    submitListing();
...
function loadServices() {
    ...
    fetch(`/get_services//${originalListingTitle}/${username}`)
    ...
    .then(result => { 
        ...
        result.forEach(service => {
            ...
            nav_text.innerText = service.name;
            ...
            service_rate.value = service.rate;
            ...
            let times = service.times.split(";");
            for (let i = 0; i < 7; i++) {
                let appt_times = times[i].split("-");
                // Add am appts 
                appt_times[0].forEach(appt => {
                    am_appt_dict[days[i]].push(appt);
                });
                // Add pm appts
                appt_times[1].forEach(appt => {
            ...
            days_row.firstElementChild.firstElementChild.click();
