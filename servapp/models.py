...
#     def filter_service(self, stype, polygon_coordinates):
#         from django.db import connection
#         with connection.cursor() as cursor:
#             cursor.execute("""
#                 SELECT *
#                 FROM servapp_service
...
#                 s = self.model(title=row[1], user= User.objects.get(id=row[8]), service_type=row[2], location=geos_point, address=row[4], description=row[5], rate=row[6], timestamp=row[7])
